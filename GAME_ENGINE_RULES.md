...
- **Unknown answers recover.** The first answer attempt against a riddle recorded as "answer unknown" starts recovery (`riddleRecoveryService.ts`), once per riddle: one more extraction from the posing narration. If it finds the answer, it is stored and the next attempt is judged normally. If not, a short DM beat closes the riddle: a narration-only turn committed under the session guard (operation kind `riddle_recovery`, waiting for any turn still resolving), with the riddle marked `abandoned` in the same commit. The active hero keeps their turn and nothing else changes. Until one of those commits, answer attempts keep getting the retryable error.
