    ...
    <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center p-8">
      ...
      <div className="max-w-4xl w-full text-center space-y-16 animate-in fade-in zoom-in duration-700">
        <div className="space-y-4">
          <h1 className="text-9xl font-black text-amber-500 drop-shadow-[0_10px_10px_rgba(0,0,0,0.5)] italic tracking-tighter">🐉 AI DM</h1>
        </div>
            ...
            {activeSessions.length > 0 && (
                <div className="w-full bg-slate-900 p-12 rounded-[60px] border-2 border-slate-800 shadow-2xl flex flex-col space-y-8">
                    ...
                    {activeSessions.map(sess => (
                        <div key={sess.id} className="w-full p-6 bg-black/40 hover:bg-amber-500/10 rounded-[32px] border-2 border-slate-800 text-amber-500 cursor-pointer flex justify-between items-center" onClick={() => navigate(`/session/${sess.id}`)}>
