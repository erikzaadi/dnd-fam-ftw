  ...
  return (
    <header className="relative z-[60] flex-shrink-0 flex justify-between items-center gap-4 px-4 py-3 border-b border-white/10 bg-slate-950/90 backdrop-blur-sm">
      ...
      <div className="flex items-center gap-2 shrink-0">
        {/* Gear popover */}
        <div ref={gearRef} className="relative group">
          <button
            onClick={() => setGearOpen(o => !o)}
            className={`w-9 h-9 flex items-center justify-center rounded-xl border text-base transition-all ${gearOpen ? 'border-amber-500/60 bg-amber-500/10 text-amber-400' : 'border-slate-700 text-slate-400 hover:border-slate-500 hover:text-slate-200'}`}
          >
            ⚙
          </button>
          {!gearOpen && (
