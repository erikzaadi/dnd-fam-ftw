...
export const Inventory = ({ party, onUseItem, onGiveItem, disabled }: InventoryProps) => {
          ...
          {partyWithItems.map(char => (
            <div key={char.id}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">{char.name}{char.status === 'downed' ? ' · downed' : ''}</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {char.inventory.map((item) => {
                  const bonuses = item.statBonuses ? Object.entries(item.statBonuses).filter(([, v]) => v && v > 0) : [];
                  const canUse = interactive && isUsable(item);
                  const canGive = interactive && isGiveable(item) && party.filter(c => c.id !== char.id).length > 0;
