...
if (out1.party[0].status !== 'active') {
  throw new Error(`Expected 'active', got '${out1.party[0].status}'`);
}
...
if (out2.party[0].status !== 'downed') {
  throw new Error(`Expected 'downed', got '${out2.party[0].status}'`);
}
...
if (out3.inventory.length !== 1) {
  throw new Error('Expected 1 inventory item');
}
if (out3.inventory[0].ownerName !== 'Pip') {
  throw new Error(`Expected ownerName 'Pip', got '${out3.inventory[0].ownerName}'`);
}
if (out3.inventory[0].healValue !== 3) {
  throw new Error(`Expected healValue 3, got ${out3.inventory[0].healValue}`);
}
if (out3.inventory[0].consumable !== true) {
  throw new Error('Expected consumable true');
}
...
if (out4.inventory.length !== 2) {
  throw new Error(`Expected 2 items, got ${out4.inventory.length}`);
}
const pipItem = out4.inventory.find(i => i.ownerName === 'Pip');
const zaraItem = out4.inventory.find(i => i.ownerName === 'Zara');
if (!pipItem) {
  throw new Error('Missing Pip item');
}
if (!zaraItem) {
  throw new Error('Missing Zara item');
}
if (zaraItem.transferable !== true) {
  throw new Error('Expected transferable true for Zara item');
}
...
if (out5.storySummary !== 'The party entered the goblin lair and upset the chef.') {
  throw new Error('storySummary not passed');
}
...
if (out6.storySummary !== undefined) {
  throw new Error(`Expected undefined, got '${out6.storySummary}'`);
}
...
if (!out7.isFirstTurn) {
  throw new Error('Expected isFirstTurn true');
}
...
if (out8.isFirstTurn) {
  throw new Error('Expected isFirstTurn false');
}
...
if (out9.recentHistory.length !== 2) {
  throw new Error(`Expected 2 history entries, got ${out9.recentHistory.length}`);
}
if (out9.recentHistory[0] !== 'Turn 1 narration.') {
  throw new Error('recentHistory[0] mismatch');
}
...
if (out10.actionResult.statUsed !== undefined) {
  throw new Error(`Expected undefined, got '${out10.actionResult.statUsed}'`);
}
...
if (out11.actionResult.summary !== 'The action failed.') {
  throw new Error(`Unexpected summary: '${out11.actionResult.summary}'`);
}
...
if (StorySummaryService.shouldUpdate(1)) {
  throw new Error('Should not update at turn 1');
}
...
for (const t of [2, 3, 4]) {
  if (StorySummaryService.shouldUpdate(t)) {
    throw new Error(`Should not update at turn ${t}`);
  }
}
...
if (!StorySummaryService.shouldUpdate(5)) {
  throw new Error('Should update at turn 5');
}
...
for (const t of [10, 15, 20]) {
  if (!StorySummaryService.shouldUpdate(t)) {
    throw new Error(`Should update at turn ${t}`);
  }
}
...
if (!out17.sanctuaryRecovery) {
  throw new Error('Expected sanctuaryRecovery true');
}
...
if (!out18.interventionRescue) {
  throw new Error('Expected interventionRescue true');
}
