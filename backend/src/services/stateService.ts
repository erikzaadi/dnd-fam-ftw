...
export class StateService {
  ...
  public static async createSession(worldDescription?: string, difficulty: string = 'normal'): Promise<SessionState> {
    ...
    for (const char of characters) {
      (char as unknown as { inventory: InventoryItem[] }).inventory = db.prepare('SELECT * FROM inventory WHERE characterId = ?').all(char.id) as InventoryItem[];
  ...
  public static async addTurnResult(id: string, turn: TurnResult, characterId: string): Promise<void> {
    const db = this.getDb();
