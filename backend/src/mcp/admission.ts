...
// same one the website uses; the per-token daily ceiling bounds a misbehaving host.
...
export const admitPaidCall = (principal: McpPrincipal, now: number = Date.now()): McpAdmission => {
  ...
  const count = authChallengeRepository.incrementRateLimit(`mcp-paid:${principal.tokenId}`, dayStart);
  if (count > ceiling) {
