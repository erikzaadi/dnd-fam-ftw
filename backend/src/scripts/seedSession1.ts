...
// Showcases: normal difficulty, fast game mode, multi-character turn sequence, basic inventory (healing potion, stat-bonus items).
...
export function seed(db: DB): void {
  ...
  db.prepare(`INSERT INTO sessions (id, scene, sceneId, displayName, turn, activeCharacterId, tone, difficulty, gameMode, savingsMode, useLocalAI, interventionUsed, rescues_used, game_over, storySummary, dm_prep)
    ...
    .run(SESSION_ID, 'The party delved into the goblin caves seeking a stolen artifact. They fought through waves of goblin sentries, discovered a map to the throne room, and now stand before the Goblin King himself.');
  ...
  seedTurn(db, SESSION_ID, 'seed-s1-c1', 'The Goblin King roars from his throne of stolen loot. "Intrudersss! KILL THEM!" He hurls a jagged crown like a disc weapon. Barnabas barely ducks in time.', CHOICES_COMBAT, 'Search the area carefully', 'mischief', 0, 4, 1, 'normal', null, 8);
  seedTurn(db, SESSION_ID, 'seed-s1-c2', 'Zara spots a hidden lever behind the throne - if she can reach it, the cage trap above the king might spring. The king is distracted by Barnabas.', CHOICES_COMBAT, 'Search the area carefully', 'mischief', 1, 19, 4, 'normal', null, 8, '🎲 A focused eye! You spot the mechanism.');
}
