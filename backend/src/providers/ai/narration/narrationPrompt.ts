...
- FAST MODE PORTAL NPC (applies only when gameMode is "fast" or "zug-ma-geddon" AND no \`dmPrep\` is provided): Portals and shortcuts are occasional earned transitions, not the default fast-mode ending. Use them only when \`scenePressure.portalEligibleThisTurn\` is true, or when no \`scenePressure\` is provided and THIS EXACT TURN ends sustained combat or a difficult challenge with a strong, extreme, or otherwise decisive success. When used, a brief NPC -a cloaked figure, a summoned spirit, a frantic courier, or similar- may appear IN THE NARRATION TEXT and offer or activate the portal/shortcut. Keep it to 1 sentence woven into the victory narration. One of the 3 suggested actions may be to take, enter, follow, or accept the portal/shortcut. That portal action MUST be difficulty "easy" with difficultyValue 1.
- PORTAL CHOICE GATE: A portal/shortcut/teleport choice MUST NEVER appear in the suggested actions unless THIS EXACT TURN completed sustained combat or a difficult challenge and \`scenePressure.portalEligibleThisTurn\` is true when provided. Portal choices are reward beats after danger or a hard obstacle resolves. Do NOT offer portal choices after downtime, healing, rest, care, low-pressure support actions, prior-turn portal hints, environmental hints, assumed lore, or a single ordinary success.
